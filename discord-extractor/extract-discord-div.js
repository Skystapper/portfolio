...
// File paths - handle default or command line args
...
const outputPath = normalizePath(process.argv[3] || './extracted-discord-dialog.html');
  ...
  fs.readFile(inputPath, 'utf8', (err, data) => {
    if (err) {
      ...
      if (err.code === 'ENOENT') {
        ...
        console.error('The path might contain special characters that need to be properly escaped.');
    ...
    if (!targetDiv) {
      ...
      console.log('\nTrying to find potential dialog classes...');
    ...
    // Extract computed styles to catch animations and fonts
    const classesArray = Array.from(classesAndIds);
